...
if __name__ == "__main__":
    print("Generating picnic rules dynamically from LLM and storing them by rule type...")
    
    for _ in range(5):
        rule_type, rule = generate_and_store_rule()
        print("\n")
    
    # Display all stored rules by rule type
    print("\nStored rules by type:")
    for rule_type, rules in rules_storage.items():
        print(f"\n{rule_type} rules:")
        for rule in rules:
            print(f"- {rule}")