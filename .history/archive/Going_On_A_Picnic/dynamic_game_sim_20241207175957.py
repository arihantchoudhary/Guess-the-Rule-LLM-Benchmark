...
def validate_examples(secret_rule, examples):
    ...
    Returns a list of booleans corresponding to whether each example is valid.
    """
    validation_prompt = f"""
...
"""
    response = client.chat.completions.create(
        model="gpt-4",  # or whichever model you prefer for validation
    ...
    if not json_match:
        # If no JSON found, assume validation failed
    ...
    try:
        ...
        # Ensure we have a boolean for each example
...
def get_validated_examples(secret_rule, num_examples=2, max_retries=3):
    """
    Generate and validate examples until all are valid or max_retries exhausted.
    """
    for attempt in range(max_retries):
        ...
        if all(validations):
            ...
            # If some are invalid, you can choose to regenerate or try partial fixes
            # For simplicity, just retry from scratch
            continue
    
    # If could not get all valid after several attempts, raise an error or return partial
    # Here we raise an error, but you could return only the valid subset if desired.
