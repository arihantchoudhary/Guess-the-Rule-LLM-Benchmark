...
def get_prompt(turns, remaining_count, positives, negatives, examples_exceeded):
    ...
    if turns == 1:
        prompt += (
            f"Let's play the game 'going on a picnic'. I will be the host. You will be a competitive player who wants to win the game in as few turns as possible but also does not want to sacrifice accuracy by giving wrong rules each turn. I will give you some examples in each turn and you have to guess the underlying rule. The rule will be common for all the examples shown throughout the whole game\n"
            f"To play the game you only do one of the following actions in a turn:\n"
            f"1. type 'more N' to request N more examples for that rule; (up to {remaining_count} available)\n"
            f"2. type the rule if you think you've guessed it; do this once you're confident about the rule\n"
